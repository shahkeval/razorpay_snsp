...
exports.razorpayWebhook = async (req, res) => {
  ...
  if (event === "payment_link.paid") {
    ...
    console.log(
      `paymentlinkid: ${paymentLinkId}, paymentId: ${paymentId}, signature: ${paymentSignature}`
    );
...
exports.updateYatrik = async (req, res) => {
  try {
    const updatedYatrik = await Yatrik.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true, runValidators: true }
    );
    if (!updatedYatrik)
      return res.status(404).json({ message: "Yatrik not found" });
...
exports.getAllPayments = async (req, res) => {
  try {
    ...
    const paymentsWithMobile = payments.map((p) => {
      let mobileNumber = "";
      if (p.yatrikNo && yatrikMap[p.yatrikNo])
        mobileNumber = yatrikMap[p.yatrikNo];
      else if (p.vaiyavachiNo && vaiyavachMap[p.vaiyavachiNo])
        mobileNumber = vaiyavachMap[p.vaiyavachiNo];
