...
exports.createVaiyavachi = [
  ...
  async (req, res) => {
    try {
      const vaiyavachiImagePath = req.file
        ? `/uploads/vaiyavach/${req.file.filename}`
        : "";
      const body = req.body;
...
exports.getVaiyavachiById = async (req, res) => {
  try {
    ...
    if (!vaiyavachi)
      return res.status(404).json({ message: "Vaiyavachi not found" });
...
exports.updateVaiyavachi = async (req, res) => {
  try {
    const updatedVaiyavachi = await Vaiyavachi.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true, runValidators: true }
    );
    if (!updatedVaiyavachi)
      return res.status(404).json({ message: "Vaiyavachi not found" });
